...
class Processo:
    def __init__(self, pid, nome):
        ...
        self.tempo_execucao = random.randint(3, 7)  # Tempo total de execução do processo
...
class Despachante:
    ...
    def executar(self):
        while self.fila:
            ...
            thread = threading.Thread(target=processo_atual.executar, args=(self.quantum,))
            print(f"\n[Despachante] Trocando para: {processo_atual.nome}")
            thread.start()  # Inicia a thread
            thread.join()  # Aguarda a thread finalizar
            if processo_atual.tempo_restante > 0:
                self.fila.append(processo_atual)  # Se não terminou, coloca de volta na fila
